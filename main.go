...
// Créer les "nbSprintes" sprites avec la logique de sprites(), en évitant les sorties d’écran
func initSprites() {
	nbSprites := 10
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	// Calculer le facteur de zoom dynamique
	cycle := math.Mod(float64(iteration), 360)  // Cycle sur 360 frames (6 secondes à 60 FPS)
	zoomAmplitude := 0.2                        // Zoom entre 0.8 et 1.2
	zoomPhase := math.Sin(toRadians(cycle / 6)) // Période de 6 secondes
	backZoom := 1.0 + zoomAmplitude*zoomPhase
	// Ajouter une pause sans zoom
	if math.Mod(float64(iteration), 480) >= 360 {
		backZoom = 1.0 // Pause de 2 secondes toutes les 8 secondes
	}
	if debug && math.Mod(iteration, 10) == 0 { // Logger toutes les 10 frames
		log.Printf("iteration: %.0f, cycle: %.2f, zoomPhase: %.2f, backZoom: %.6f", iteration, cycle, zoomPhase, backZoom)
	}
	...
	if debug && math.Mod(iteration, 10) == 0 {
		log.Printf("decalX: %.6f", decalX)
	}
	...
	for ligne := 0; ligne < screenHeight; ligne++ {
		...
		op := &ebiten.DrawImageOptions{}
		// Calculer la largeur ajustée pour couvrir screenWidth après le zoom
		adjustedWidth := int(float64(screenWidth) / backZoom)
		// Appliquer le zoom et centrer l’image
		op.GeoM.Scale(backZoom, 1.0) // Zoom uniquement en X
		// Calculer le décalage pour centrer l’image après zoom
		renderedWidth := float64(adjustedWidth) * backZoom
		offsetX := (float64(screenWidth) - renderedWidth) / 2
		op.GeoM.Translate(offsetX, float64(ligne)) // Centrer en X, placer à la bonne ligne
		// Ajuster le rectangle pour couvrir toute la largeur après le zoom
		backRect := image.Rect(backX, int(math.Mod(float64(ligne)+bounceBack, backHeight)), backX+adjustedWidth, int(math.Mod(float64(ligne)+bounceBack, backHeight))+1)
		if debug && math.Mod(iteration, 10) == 0 {
			log.Printf("backCanvas SubImage rect: %v, adjustedWidth: %d, renderedWidth: %.2f, offsetX: %.2f", backRect, adjustedWidth, renderedWidth, offsetX)
		}
		// Vérifier les limites de backCanvas
		if backRect.Max.X > backCanvas.Bounds().Max.X {
			backRect.Max.X = backCanvas.Bounds().Max.X
			if debug {
				log.Printf("Clamped backRect.Max.X to %d", backRect.Max.X)
			}
		}
		mainCanvas.DrawImage(backCanvas.SubImage(backRect).(*ebiten.Image), op)
		if debug && math.Mod(iteration, 10) == 0 {
			log.Printf("Drew backCanvas line %d, backX: %d, backZoom: %.6f", ligne, backX, backZoom)
		}
...
func main() {
	...
	*/
	backCanvas = ebiten.NewImage(screenWidth*2, backHeight) // 832x64 au lieu de 424x64
	if debug {
		log.Printf("backCanvas initialized, size: %v", backCanvas.Bounds().Size())
	}
